...
inline BT::NodeStatus RemovePassedGoals::tick()
{
  ...
  Goals goal_poses;
  Goals last_goal_poses_;
  getInput("input_goals", goal_poses);

  if (goal_poses != last_goal_poses_) {
    first_tick_ = false;  
    last_goal_poses_ = goal_poses;
  } 
  ...
  if (!first_tick_) {
    ...
    for (size_t i = 0; i < goal_poses.size(); ++i) {
      double dist = euclidean_distance(current_pose.pose, goal_poses[i].pose);
    ...
    goal_poses.erase(goal_poses.begin(), goal_poses.begin() + closest_pose_idx);
    setOutput("output_goals", goal_poses);
    first_tick_ = true;
  }
