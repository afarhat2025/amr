...
class BasicNavigator(Node):
    ...
    def __init__(self, node_name='basic_navigator', namespace=''):
        ...
        self.clear_global_costmap_around_robot_srv = self.create_client(
            ClearCostmapAroundRobot, 'global_costmap/clear_around_global_costmap'
        )
        self.clear_local_costmap_around_robot_srv = self.create_client(
            ClearCostmapAroundRobot, 'local_costmap/clear_around_local_costmap'
        )
        self.clear_costmap_except_region_srv = self.create_client(
            ClearCostmapExceptRegion, 'local_costmap/clear_except_local_costmap'
        )
    ...
    def clearlocalCostmapAroundRobot(self, reset_distance: float):
        """Clear the costmap around the robot."""
        while not self.clear_local_costmap_around_robot_srv.wait_for_service(timeout_sec=1.0):
            self.info('ClearLocalCostmapAroundRobot service not available, waiting...')
        req = ClearCostmapAroundRobot.Request()
        req.reset_distance = reset_distance
        future = self.clear_local_costmap_around_robot_srv.call_async(req)
        rclpy.spin_until_future_complete(self, future)
        return
    
    def clearglobalCostmapAroundRobot(self, reset_distance: float):
        """Clear the costmap around the robot."""
        while not self.clear_global_costmap_around_robot_srv.wait_for_service(timeout_sec=1.0):
            self.info('ClearGlobalCostmapAroundRobot service not available, waiting...')
        req = ClearCostmapAroundRobot.Request()
        req.reset_distance = reset_distance
        future = self.clear_global_costmap_around_robot_srv.call_async(req)
        rclpy.spin_until_future_complete(self, future)
        return
